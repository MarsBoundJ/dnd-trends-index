...
def google_autopsy():
    ...
    # Correct path is dnd_trends_categorized.trend_data_pilot
    sql = """
...
def amazon_check():
    print("\n--- Amazon ASIN Map Check ---")
    sql = "SELECT * FROM `dnd-trends-index.dnd_trends_categorized.amazon_asin_map`"
    try:
        query_job = client.query(sql, location="US")
        results = query_job.to_dataframe()
        print(f"ASIN Map Rows: {len(results)}")
        print(results.to_string(index=False))
    except Exception as e:
        print(f"Error in Amazon Check: {e}")
