...
print("Creating Market Equilibrium View...")
try:
...
try:
    # #1 Hype Bubble
    bubble = list(client.query("SELECT concept_name FROM `dnd-trends-index.gold_data.study_market_equilibrium` WHERE classification = 'Hype Bubble' ORDER BY hype_ratio DESC LIMIT 1").result())
    if bubble:
        print(f"#1 Hype Bubble: {bubble[0].concept_name}")
    else:
        print("#1 Hype Bubble: None found (No concepts meet 'High Social / Low Trend' threshold)")

    # #1 Quiet Giant
    giant = list(client.query("SELECT concept_name FROM `dnd-trends-index.gold_data.study_market_equilibrium` WHERE classification = 'Quiet Giant' ORDER BY hype_ratio ASC LIMIT 1").result())
    if giant:
        print(f"#1 Quiet Giant: {giant[0].concept_name}")
    else:
