...
**Current phase:** Step 9.10 complete — **The Dean** (8th Council voice) + `frames/industry-fundamentals` + Track C routing shipped end-to-end. Voice block synthesized from Thompson (Stratechery) + Galloway (NYU Stern / No Mercy) + Christensen (HBR canon) + HBR/McKinsey composite, with explicit per-register triggers and an embedded Layer-2 beat lexicon (Porter's Five Forces, disruption theory, Aggregation Theory, Blue Ocean, Rundle, CAC/LTV). Frame-gated activation: Dean fires only when `industry-fundamentals` is the active frame — parallel to Gary's demand-side route and Chronicler's `mode=chronicler` path, NOT in the weekday rotation pool. First live article ("Core Mechanic Hype Lacks Conversion to Play") graded 4/6 clear pass on the acceptance criterion stashed in `project_council_voice_tuning.md`. Two v1.1 tuning items logged (infrastructure-invisibility rule + framework-label discipline), non-blocking. Also in this PR: shared `lib/disclaimer.ts` module powering article footer + Sage chat footer + Council bio popover disclaimers; the reference Dean article (hand-crafted cannibalization thesis) published live; the Dean sigil (GraduationCap) + `CouncilAuthorName` type registered in the frontend.
**Next step:** 9.10.1 (Dean v1.1 tuning — folds the two observed issues), 9.8b (co-byline prompt generation), 9.9.5 (trending-wikis auto-discovery), 9.9.6 (non-D&D Reddit + YouTube ingestion), 9.11 (Reports format). Step 12.5 (legacy admin port) and Step 13 (Aceternity) land after the 9.x series.
