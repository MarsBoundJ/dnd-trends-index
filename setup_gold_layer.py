...
def setup_gold():
    ...
    WITH universe AS (
        SELECT date, keyword FROM `{PROJECT_ID}.silver_data.norm_wikipedia`
        UNION DISTINCT
        SELECT date, keyword FROM `{PROJECT_ID}.silver_data.norm_fandom`
        UNION DISTINCT
        SELECT date, keyword FROM `{PROJECT_ID}.silver_data.norm_roll20`
        UNION DISTINCT
        SELECT date, keyword FROM `{PROJECT_ID}.silver_data.norm_youtube`
        UNION DISTINCT
        SELECT date, search_term as keyword FROM `{PROJECT_ID}.dnd_trends_categorized.trend_data_pilot`
    ),
    daily_scores AS (
        SELECT 
            u.date,
        ...
        FROM universe u
        LEFT JOIN `{PROJECT_ID}.silver_data.norm_wikipedia` w
            ON u.date = w.date AND u.keyword = w.keyword
        LEFT JOIN `{PROJECT_ID}.silver_data.norm_fandom` f
            ON u.date = f.date AND u.keyword = f.keyword
        LEFT JOIN `{PROJECT_ID}.silver_data.norm_roll20` r
            ON u.date = r.date AND u.keyword = r.keyword
        LEFT JOIN `{PROJECT_ID}.silver_data.norm_youtube` y
            ON u.date = y.date AND u.keyword = y.keyword
        LEFT JOIN `{PROJECT_ID}.dnd_trends_categorized.trend_data_pilot` g
    ...
    LEFT JOIN (
        ...
        FROM `{PROJECT_ID}.dnd_trends_categorized.expanded_search_terms`
