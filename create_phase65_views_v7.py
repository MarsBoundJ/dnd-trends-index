...
WITH raw_data AS (
    ...
    JOIN `dnd-trends-index.dnd_trends_categorized.expanded_search_terms` AS m ON t.term_id = m.term_id
    JOIN `dnd-trends-index.dnd_trends_categorized.concept_library` AS lib ON m.concept_name = lib.concept_name
),
...
WITH persona_interest AS (
    ...
    JOIN `dnd-trends-index.dnd_trends_categorized.expanded_search_terms` AS m ON t.term_id = m.term_id
    JOIN `dnd-trends-index.dnd_trends_categorized.concept_library` AS lib ON m.concept_name = lib.concept_name
    WHERE lib.persona_target IN ('DM', 'PLAYER')
    GROUP BY 1, 2
),
pivoted AS (
    SELECT 
