/**
 * DMs Guild / DriveThruRPG Commercial-Supply Bookmarklet ("Incursion V9")
 * (Stage 8 of the community_reception multi-source composite, Apr 30, 2026)
 *
 * PROVENANCE: this file was recovered on Sep 1, 2026 by decoding the installed
 * "DMsGuild / DTRPG Incursion" browser bookmark. The bookmarklet shipped in
 * Apr 2026 but its source was never committed, so the browser copy was the only
 * copy. Body below is byte-for-byte what was installed; only this header is new.
 *
 * Usage:
 *   1. Browse to a DMs Guild or DriveThruRPG listing page — either a metal
 *      best-seller shelf (`metal.php`) or any regular browse/category page.
 *   2. Click the bookmarklet. It scans every `a[href*="/product/"]` on the
 *      page, dedupes by product URL, and reads title / price / shelf tier.
 *   3. A panel shows the detected count; click "Transmit Batch" to POST in
 *      chunks of 500 to /system/library/ingest-catalog on the bouncer.
 *
 * Site is auto-detected from hostname → `source` is "DMs Guild" or
 * "DriveThruRPG"; both are members of the bouncer's TTRPG_SOURCES set, so rows
 * land in dnd_trends_raw.catalog_supply alongside the Amazon harvest.
 *
 * Shelf tiers (Adamantine → Copper) are DMs Guild's sales-rank bands. They are
 * resolved by walking previous siblings, then ancestors, for a heading naming a
 * tier — the tier is a section header, not a per-product attribute. Products
 * outside any tiered shelf are recorded as "Normal".
 *
 * Note: unlike the fully-automatic Amazon harvester, this one is deliberately
 * two-step — scan, then an explicit human click to transmit.
 */
