...
stable_latest_dates AS (
    -- Find the latest date for each category that has a significant volume (at least 80% of peak)
    -- This ignores sparse "canary" or "pilot" loads that distort metrics.
    SELECT 
        ds.category, 
...
ranked AS (
    SELECT 
        ...
        m.canonical_concept,
        m.google_score_avg,
        -- Use ROW_NUMBER for exactly 20 items in heat calculation
