...
def main():
    print("Surgical Recovery: Lore & NPCs Category")
    
    # 1. Expand search terms
    print("Fetching Lore concepts...")
    query = f"SELECT concept_name, category FROM `{SOURCE_TABLE}` WHERE category = 'Lore & NPCs' AND is_active = TRUE"
    concepts = list(client.query(query).result())
    
    existing_query = f"SELECT original_keyword, search_term FROM `{EXPANDED_TABLE}` WHERE category = 'Lore & NPCs'"
    ...
    for concept in concepts:
        ...
        # Rule: Standalone (critical for Lore)
        targets = [name, f"{name} Dnd"]
        for t in targets:
