...
def main():
    print("Fetching Battle Maps & Tactical Surfaces concepts...")
    query = f"""
        SELECT concept_name, category 
        FROM `{SOURCE_TABLE}`
        WHERE category = 'Battle Maps & Tactical Surfaces' AND is_active = TRUE
    """
    concepts = list(client.query(query).result())
    print(f"Found {len(concepts)} active Battle Maps concepts.")

    print("Checking for existing expansions...")
    existing_query = f"SELECT original_keyword, search_term FROM `{DEST_TABLE}` WHERE category = 'Battle Maps & Tactical Surfaces'"
    ...
    for concept in concepts:
        ...
        # Rule 3: [Term] 5e
        suffix_5e = f"{name} 5e"
        if (name, suffix_5e) not in existing_map:
            new_rows.append({
                "term_id": str(uuid.uuid4()),
                "original_keyword": name,
                "category": cat,
                "search_term": suffix_5e,
                "expansion_rule": "suffix_5e_recovery",
                "created_at": datetime.datetime.now().isoformat(),
                "is_pilot": True
            })
