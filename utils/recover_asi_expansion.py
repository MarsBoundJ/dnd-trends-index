...
def main():
    print("Surgical Recovery: Ability Score Improvement Signal Expansion", flush=True)
    
    query = f"""
    SELECT term_id, search_term 
    FROM `{PROJECT_ID}.{DATASET_ID}.expanded_search_terms`
    WHERE original_keyword = 'Ability Score Improvement'
    AND search_term IN ('ASI 5e', '5e asi', 'asi dnd', 'dnd asi')
    """
    
    terms = list(client.query(query).result())
    if not terms:
        print("No new terms found to recover.")
        return
    ...
    for i in range(0, len(search_list), batch_size):
        ...
        if not success:
            print("    Retrying batch...", flush=True)
            time.sleep(10)
            success = fetch_batch(batch_kws, term_map, dest_table)
