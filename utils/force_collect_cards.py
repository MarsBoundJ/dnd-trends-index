...
def main():
    print("Surgical Recovery: Force Collect Cards & Reference Materials Trends")
    
    # query to get terms for Cards & Reference Materials
    # Use DISTINCT to avoid the triplicated terms issue
    query = f"""
        SELECT e.search_term, ANY_VALUE(e.term_id) as term_id
        FROM `{SOURCE_TABLE}` e
        JOIN `{PROJECT_ID}.{DATASET_ID}.concept_library` c ON e.original_keyword = c.concept_name
        WHERE c.category = 'Cards & Reference Materials' AND c.is_active = TRUE
        GROUP BY 1
    """
