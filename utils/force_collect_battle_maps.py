...
def main():
    print("Surgical Recovery: Force Collect Battle Maps Trends")
    
    # query to get terms for Battle Maps & Tactical Surfaces
    query = f"""
        SELECT DISTINCT e.search_term 
        FROM `{SOURCE_TABLE}` e
        JOIN `{PROJECT_ID}.{DATASET_ID}.concept_library` c ON e.original_keyword = c.concept_name
        WHERE c.category = 'Battle Maps & Tactical Surfaces' AND c.is_active = TRUE
    """
    rows = list(client.query(query).result())
    print(f"Total unique terms to fetch: {len(rows)}")

    # We need to map search_term to A term_id for the insert
    term_lookup_query = f"""
        SELECT search_term, term_id 
        FROM `{SOURCE_TABLE}` 
        WHERE category = 'Battle Maps & Tactical Surfaces'
    """
    term_lookup_rows = list(client.query(term_lookup_query).result())
    term_map = {r['search_term']: r['term_id'] for r in term_lookup_rows}

    # Small batches for high fidelity
    BATCH_SIZE = 2 
