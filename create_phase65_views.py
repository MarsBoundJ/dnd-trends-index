...
WITH raw_data AS (
    ...
    JOIN `dnd-trends-index.dnd_trends_categorized.expanded_search_terms` m ON t.search_term = m.google_keyword
),
...
ruleset_daily_stats AS (
    SELECT 
        ...
        r.category,
        r.ruleset_tag,
        SUM(r.interest) as ruleset_interest
    FROM raw_data r
    GROUP BY 1, 2, 3
)
SELECT 
...
WITH persona_interest AS (
    ...
    FROM `dnd-trends-index.dnd_trends_categorized.trend_data_pilot` t
    JOIN `dnd-trends-index.dnd_trends_categorized.expanded_search_terms` m ON t.search_term = m.google_keyword
    JOIN `dnd-trends-index.dnd_trends_categorized.concept_library` c ON m.concept_name = c.concept_name
